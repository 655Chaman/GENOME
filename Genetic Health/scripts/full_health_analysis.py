    ...
    with open(output_path, 'w') as f:
        ...
        if high_impact:
            for finding in high_impact:
                f.write(f"### {finding['gene']}\n")
                f.write(f"**🧬 Search Terms:** {finding['gene']}, {finding['rsid']}, {finding['category']}, {finding['status'].replace('_', ' ')}\n\n")
                f.write(f"**💡 Plain English Summary:** {finding['description']}\n")
                if finding['note']:
        ...
        for category, emoji, description in category_order:
            if category in results['by_category']:
                ...
                f.write("| Gene | Genotype | Status | What this means |\n")
                f.write("|------|----------|--------|-----------------|\n")
                ...
                for finding in findings_sorted:
                    ...
                    f.write(f"| {finding['gene']} | `{finding['genotype']}` | {status} {impact_indicator} | {desc} |\n")
        ...
        if level_1:
            ...
            for finding in level_1[:20]:
                f.write(f"**{finding['gene']}**\n")
                f.write(f"**🧬 Search Terms:** {finding['gene']}, {finding['rsid']}, {finding['drugs']}\n")
                f.write(f"- **Drugs Affected:** {finding['drugs']}\n")
                annotation = finding['annotation']
                if len(annotation) > 300:
                    annotation = annotation[:297] + "..."
                f.write(f"- **What this means:** {annotation}\n\n")
