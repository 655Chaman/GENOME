...
**💡 Plain English Summary:** These variants are classified as pathogenic and your genotype suggests you may be affected.
**✅ What You Should Do:** Consult a genetic counselor or physician for clinical interpretation.

"""
        for f in affected_findings:
            stars = '⭐' * f['gold_stars'] + '☆' * (4 - f['gold_stars'])
            report += f"""### {f['gene']}
**🧬 Search Terms:** {f['gene']}, {f['rsid']}, chr{f['chromosome']}:{f['position']}, {f['traits']}
...
**💡 Plain English Summary:** You are a heterozygous carrier for these autosomal recessive conditions.
Carriers typically do not show symptoms but may pass the variant to offspring.

**⚠️ Reproductive Implications:**
...
"""
        for f in carrier_findings:
            ...
            report += f"""### {f['gene']}
**🧬 Search Terms:** {f['gene']}, {f['rsid']}, chr{f['chromosome']}:{f['position']}, {condition}
**Condition:** {condition}
...
"""
    ...
    if het_unknown_findings:
        report += """## ⚠️ Pathogenic/Likely Pathogenic — Inheritance Unclear

**💡 Plain English Summary:** You have these variants, but the inheritance pattern is not clearly specified,
so clinical impact is uncertain. Some may mean you are affected, others may just mean you are a carrier.
...
**💡 Plain English Summary:** These variants are associated with increased susceptibility to certain conditions.
...
"""
        for f in findings['risk_factor']:
            stars = '⭐' * f['gold_stars'] + '☆' * (4 - f['gold_stars'])
            report += f"""### {f['gene']}
**🧬 Search Terms:** {f['gene']}, {f['rsid']}, {f['traits']}
**Condition:** {f['traits'] if f['traits'] else 'Not specified'}
...
**💡 Plain English Summary:** These variants affect response to medications. Tell your doctor before taking new drugs.
...
"""
    ...
    if findings['protective']:
        report += """## 🛡️ Protective Variants

**💡 Plain English Summary:** These variants are associated with reduced disease risk or protective effects.
