...
def generate_finding_section(finding, index):
    """Generate a comprehensive section for a single finding in a simple format."""
    ...
    mag_text = format_magnitude(magnitude)

    section = []
    section.append(f"### {index}. {gene} {mag_text}")
    section.append("")
    section.append(f"**🧬 Search Terms (For Googling):** {gene}, {rsid}, {status.replace('_', ' ')}")
    section.append("")
    section.append(f"**💡 Plain English Summary:**")
    ...
    if context:
        section.append("")
        section.append(f"*{context['mechanism']}*")
        
        if context.get('implications'):
            section.append("")
            section.append("**⚠️ What This Means For You:**")
            for imp in context['implications']:
                section.append(f"- {imp}")

        if context.get('actions'):
            section.append("")
            section.append("**✅ What You Should Do:**")
...
def generate_pharmgkb_section(finding, index):
    ...
    section = []
    section.append(f"### {index}. {gene} - Drug Interaction")
    section.append("")
    section.append(f"**🧬 Search Terms (For Googling):** {gene}, {rsid}, {drugs}, {genotype}")
    section.append("")
    section.append(f"**💊 Drugs Affected:** {drugs}")
    section.append("")
    section.append("**💡 Plain English Summary:**")
    section.append(f"{annotation}")
    section.append("")

    # Add drug-specific guidance based on level and category
    if level in ["1A", "1B"]:
        section.append("**✅ What You Should Do:**")
        section.append("- **Tell your doctor** before taking these medications. Your body processes them differently.")
...
def generate_full_findings(findings):
    ...
    for category in categories:
        ...
        for i, finding in enumerate(cat_findings, 1):
            ...
            lines.append(f"#### {i}. {gene} {mag_icon}")
            lines.append(f"**🧬 Search Terms (For Googling):** {gene}, {rsid}, {status}, {genotype}")
            lines.append(f"**💡 Plain English Summary:** {description}")

            context = get_clinical_context(gene, finding.get('status', ''))
            if context:
                lines.append(f"**⚠️ What This Means:** {context['mechanism'][:200]}...")
                if context.get('actions'):
